...
function getPolynom(...args) {
  return (x) => {
    ...
    return args.reduceRight((res, item, index) => res + x ** index * args[args.length - index - 1],
      0);
  };
}
...
function getArgsString(args) {
  let argsStr = '';
  args.forEach((arg, indx) => {
    if (Array.isArray(arg)) argsStr += `[${getArgsString(arg)}]`;
    if (typeof arg === 'string') argsStr += `"${arg}"`;
    if (typeof arg === 'number') argsStr += `${arg}`;
    if (indx !== args.length - 1) argsStr += ',';
  });
  return argsStr;
}
